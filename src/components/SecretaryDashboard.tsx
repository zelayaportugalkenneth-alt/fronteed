...
function SecretaryDashboard() {
  ...
  const loadInterviews = () => setInterviews(getInterviews());
      ...
      <div className="requests-layout">
        <aside className="reviewed-panel" aria-labelledby="reviewed-title">
          <div className="panel-heading">
            <div><p className="eyebrow">Historial</p><h2 id="reviewed-title">Revisadas</h2></div>
            <span className="count-badge">{reviewed.length}</span>
          </div>
          <div className="reviewed-list">
            {reviewed.length === 0 ? <p className="empty-requests">Todavía no hay entrevistas revisadas.</p> : reviewed.map((interview) => <RequestCard key={interview.id} interview={interview} />)}
          </div>
        </aside>

        <section className="active-panel" aria-labelledby="active-title">
          <div className="panel-heading">
            <div><p className="eyebrow">Bandeja de entrada</p><h2 id="active-title">Solicitudes pendientes</h2></div>
            <span className="count-badge">{pending.length}</span>
          </div>
          <div className="active-list">
            {pending.length === 0 ? <p className="empty-requests">No hay solicitudes pendientes.</p> : pending.map((interview) => <RequestCard key={interview.id} interview={interview} />)}
          </div>
        </section>
      </div>
    </main>
